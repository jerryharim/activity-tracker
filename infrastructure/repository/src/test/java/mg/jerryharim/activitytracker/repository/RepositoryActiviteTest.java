...
public class RepositoryActiviteTest {
    ...
    private boolean ajouterMotCle(String motCle) {
        ...
        ReposioryActivite repositoryActivite = new ReposioryActivite();

        // when
        boolean ajoutMotCleOk = repositoryActivite.ajouterMotCle("productivite");

        // then
        then(ajoutMotCleOk).as("Ajout mot cle devrait etre ok").isTrue();
